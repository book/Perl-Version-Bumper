...
Perl::Version::Bumper::Rationale - or: How I Learned to Stop Worrying and Love the Bump

=head1 TL;DR:

Forget about C<use strict; use warnings;>, just C<use VERSION;>!
...
or simply run this (the interpreter version is available in two different
...
After this introduction, you should pay attention to the case of words
in the rest of this document.  As explained above, C<perl> (lowercase,
monospace) is the I<binary>, i.e. the actual program that compiles and
runs your Perl code, whereas Perl (initial capital) is the I<language>.

The version of the language is a somewhat more complicated topic,
as we'll explore below.

=head2 Perl features

As you may know, Perl is extremely backwards compatible. This means that
...
Bundles have also been used to deprecate and disabled features that have
...
=head2 Line 1 semantics

As explained above, C<use VERSION> automatically loads the corresponding
...
=head2 Declaring a baseline of the Perl language
...
=head1 The Once and Future Perl
...
=head2 The new "use strict and warnings"
...
=head2 Perl Maintenance Policy

The L<Perl 5 Porters|perlpolicy/Perl 5 Porters> "officially" support
the two most recent stable release series.

See also L<perlpolicy/BACKWARD COMPATIBILITY AND DEPRECATION>.

=head2 Toolchain Maintenance Policy

To quote L<The Lyon Amendment to the Lancaster
Consensus|https://github.com/Perl-Toolchain-Gang/toolchain-site/blob/master/lyon-amendment.md>:

I<
No new release of a distribution in the Perl toolchain will specify a
minimum perl prerequisite version (whether configure, build, runtime,
or test) that has been available for less than ten years.
>

With stable perl version having even minor numbers, this means that once
Perl v5.40 has been published, the I<maximum> version a toolchain module
will require is v5.20. Toolchain authors are usually quite conservatives,
so you'll definitely see requirements for older versions of the language.
